...
def create_train_val_slice(image_datasets, sample_size=None, val_same_as_train=False):
    ...
    image_datasets_reduced = {
        x: torch.utils.data.Subset(image_datasets['train' if val_same_as_train else x], sample_n['train'])
        for x in ['train', 'val']}
...
data, dataset_sizes = create_train_val_slice(image_datasets, sample_size=5)
